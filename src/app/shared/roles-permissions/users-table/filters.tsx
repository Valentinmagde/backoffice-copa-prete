...
export default function Filters<TData extends Record<string, any>>({
  ...
  const handleRoleFilter = (selectedRoles: string[]) => {
    const roleColumn = table.getColumn('role');
    
    // IMPORTANT: Utiliser un filtre personnalisé pour la correspondance exacte
    roleColumn?.setFilterValue(selectedRoles);
  };
        ...
        >
          <StatusField
            ...
            displayValue={(selected: string) =>
              renderOptionDisplayValue(statusOptions.find(s => s.value == selected)?.label as string)
            }
          />
          <StatusField
            ...
            className="@4xl:w-40"
            displayValue={(selected: string) =>
              renderOptionDisplayValue(roleOptions.find(s => s.value == selected)?.label as string)
            }
          />
          {isFiltered && (
            <Button
